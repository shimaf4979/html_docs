...
const getMDNUrl = (tagInfo: TagInfo) => {
  const tag = tagInfo.tag.replace(/[<>]/g, "");

  // 特殊なタグの処理
  if (tag === "!DOCTYPE html") {
    return "https://developer.mozilla.org/ja/docs/Glossary/Doctype";
  }

  // グローバル属性の処理
  if (tag === "すべての要素") {
    return "https://developer.mozilla.org/ja/docs/Web/HTML/Global_attributes";
  }

  // ARIA属性の処理
  if (tag === "ARIA属性") {
    return "https://developer.mozilla.org/ja/docs/Web/Accessibility/ARIA/Attributes";
  }

  // 通常のタグの処理
  return `https://developer.mozilla.org/ja/docs/Web/HTML/Element/${tag}`;
};
